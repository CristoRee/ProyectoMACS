...
class Chat {
    ...
    public function obtenerOCrearConversacion($id_ticket) {
        $stmt = $this->db->prepare("SELECT id_conversacion FROM Conversaciones WHERE id_ticket = ?");
        ...
        if ($resultado->num_rows > 0) {
            ...
            $stmt_insert = $this->db->prepare("INSERT INTO Conversaciones (id_ticket) VALUES (?)");
    ...
    public function obtenerInfoTicketParaChat($id_ticket) {
        $sql = "SELECT 
                    t.id_cliente, t.id_tecnico_asignado,
    ...
    public function obtenerConversacionesActivas($id_usuario, $rol_usuario) {
        ...
        $sql = "SELECT DISTINCT
                ...
                {$extra_join_condition}
                WHERE t.id_cliente = ? OR t.id_tecnico_asignado = ?";
